...
from app.shared.utils import _load_backlog_json, load_query

from . import work_items_bp

_PARENT_TYPES = ("project", "syndication", "foreignloan")

def _sync_project_status(conn, project_id):
    # Same logic as in okrs_bp, needs to sync Project Status when WorkItems change
    from app.okrs.routes import _sync_project_status as _okrs_sync
...
def global_backlog():
    ...
    foreignloans = conn.execute(
        "SELECT m.DealId, c.CustomerName + ' / Foreign Loan #' + CAST(m.DealId AS NVARCHAR) AS DisplayName "
        "FROM BOA.STR.MainDeals m "
        "JOIN BOA.CUS.Customer c ON m.CustomerId = c.Customerid "
        "WHERE m.ProductCode = 'FOREIGNLOAN' "
        "ORDER BY m.DealId"
    ).fetchall()
    conn.close()
