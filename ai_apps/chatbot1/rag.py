...
def _load_cache() -> tuple:
    ...
    with _cache_lock:
        if _cache:
            return _cache["texts"], _cache["matrix"], _cache["bm25"]
        ...
        rows = con.execute("SELECT text, embedding FROM chunks").fetchall()
        con.close()

        if not rows:
            _cache = {"texts": [], "matrix": np.empty((0, 0), dtype=np.float32), "bm25": None}
        ...
        _cache = {"texts": texts, "matrix": matrix, "bm25": bm25}
        print("[rag] Cache ready.", flush=True)
        return texts, matrix, bm25
...
def search_document_hybrid(user_query: str, n_results: int = 20) -> dict:
    ...
    bm25_ranked  = [texts[i] for i in np.argsort(bm25_scores)[::-1]]
